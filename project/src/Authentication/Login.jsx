...
import { useNavigate } from "react-router-dom";
import { Mail, Lock } from "lucide-react";
import { UserContext } from "../components/context/user"; // 👈 import context
...
const LoginForm = ({ switchToSignup, switchToForgotPassword }) => {
  ...
  const [error, setError] = useState("");
  const navigate = useNavigate();
  const { login } = useContext(UserContext); // 👈 context se login function
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      // Optional: fetch user from cookie
      const userRes = await axios.get("http://localhost:3000/api/auth/me", {
        withCredentials: true,
      });

      console.log("User:", userRes.data);
      login(userRes.data); // 👈 save user globally
  ...
  return (
    <form
      ...
      className="max-w-md mx-auto p-6 bg-white shadow-xl rounded-xl mt-10 space-y-6"
    >
      <h2 className="text-2xl font-bold text-center">Login</h2>
      {error && <p className="text-red-600 text-sm">{error}</p>}
